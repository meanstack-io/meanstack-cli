...
function cloneApplication(name, options, done) {
    ...
    var version = options.branch ? util.format('-b %s ', options.branch) : '';
    var cloningWith = options.git ? 'git@github.com:' : 'https://github.com/';
    var source = util.format('git clone %s%s%s %s', version, cloningWith, 'meanstack-io/meanstack.io.git', name);
